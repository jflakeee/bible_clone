'use client';

import { useVocabulary } from '@/hooks/useVocabulary';
import { StrongsEntry } from '@/types/bible';

interface AddToVocabularyButtonProps {
  entry: StrongsEntry;
  size?: 'sm' | 'md';
}

export default function AddToVocabularyButton({
  entry,
  size = 'sm',
}: AddToVocabularyButtonProps) {
  const { isInVocabulary, toggleByStrongsNumber } = useVocabulary();

  const saved = isInVocabulary(entry.number);

  const sizeClasses = size === 'sm' ? 'h-5 w-5' : 'h-6 w-6';

  return (
    <button
      onClick={(e) => {
        e.stopPropagation();
        toggleByStrongsNumber(entry);
      }}
      className="rounded-md p-1 transition-colors hover:bg-gray-100 dark:hover:bg-gray-700"
      aria-label={saved ? '단어장에서 제거' : '단어장에 추가'}
      title={saved ? '단어장에서 제거' : '단어장에 추가'}
    >
      <svg
        className={`${sizeClasses} ${
          saved
            ? 'fill-yellow-400 text-yellow-400'
            : 'fill-none text-gray-400 hover:text-yellow-400'
        }`}
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={2}
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"
        />
      </svg>
    </button>
  );
}
